...
#define DWIN_LANGUAGE_EEPROM_ADDRESS 0x01 
...
{
  BL24CXX_Read(DWIN_LANGUAGE_EEPROM_ADDRESS+2, (uint8_t*)&first_load_language, sizeof(first_load_language));
  delay(10);
  if(first_load_language == 0)
  {
    BL24CXX_Read(DWIN_LANGUAGE_EEPROM_ADDRESS, (uint8_t*)&language_change_font, 1);
  }
  else
  {
    first_load_language = 0;
    BL24CXX_Write(DWIN_LANGUAGE_EEPROM_ADDRESS+2, (uint8_t*)&first_load_language, sizeof(first_load_language));
    #if ENABLED(DEFAULT_LANGUAGE)
      language_change_font = 1;
    #else
      language_change_font = 0;
    #endif
    BL24CXX_Write(DWIN_LANGUAGE_EEPROM_ADDRESS, (uint8_t*)&language_change_font, 1);
  }
}
